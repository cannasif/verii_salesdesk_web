...
export function orderNotesGetDtoToDto(get: OrderNotesGetDto | null): QuotationNotesDto {
  if (!get) {
...
export function orderNotesDtoToNotesList(notes: QuotationNotesDto): string[] {
  const list: string[] = [];
  for (const key of NOTE_KEYS) {
