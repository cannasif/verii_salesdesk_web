...
import {
  ERP_CUSTOMER_FILTER_COLUMNS,
  getOperatorsForColumn,
  getDefaultOperatorForColumn,
} from '../types/erp-customer-filter.types';
import { Plus, Search, Trash2 } from 'lucide-react';

export interface ErpCustomerAdvancedFilterProps {
  draftRows: ErpCustomerFilterRow[];
  onDraftRowsChange: (rows: ErpCustomerFilterRow[]) => void;
  onSearch: () => void;
  onClear: () => void;
  embedded?: boolean;
}
...
export function ErpCustomerAdvancedFilter({
  draftRows,
  onDraftRowsChange,
  onSearch,
  onClear,
  embedded = false,
}: ErpCustomerAdvancedFilterProps): ReactElement {
  const { t } = useTranslation(['erp-customer-management']);

  const addRow = (): void => {
    onDraftRowsChange([
      ...draftRows,
      { id: generateId(), column: 'customerCode', operator: 'Contains', value: '' },
    ]);
  };
  ...
  const updateRow = (id: string, patch: Partial<Omit<ErpCustomerFilterRow, 'id'>>): void => {
    onDraftRowsChange(
      draftRows.map((r) => {
        if (r.id !== id) return r;
        const next = { ...r, ...patch };
          ...
          {draftRows.map((row) => (
            <div key={row.id} className="flex flex-wrap items-center gap-2">
              ...
              >
                ...
                <SelectContent>
                  {ERP_CUSTOMER_FILTER_COLUMNS.map((c) => (
